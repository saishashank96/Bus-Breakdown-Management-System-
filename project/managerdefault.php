...
<?php
	...
	$breakcount=$db->breakdown->aggregateCursor([['$group'=>['_id'=>'$Bus_No','count'=>['$sum'=>1]]],['$sort'=>['_id'=>1]]]);
	foreach($breakcount as $doc)
	{
		if($doc['count']>2)
		{
			echo "Bus ".$doc['_id']." has broken down ".$doc['count']." times ";
		}
